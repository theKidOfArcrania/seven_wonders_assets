...
COST_SHADE = 4
COST_GAP = (COST_BW - COST_SHADE) / 2.0 - COST_R   # vertical gap == left/right margin
...
def hang_ribbon(x0, top, slots, ribbon_color, icons):
    ...
    if not slots:
        return ''
    d = 2 * COST_R
    cx = x0 + (COST_BW - COST_SHADE) / 2.0  # centre on the lit ribbon body

    ys = []                           # vertical centres, one per slot
    y = top + COST_GAP + COST_R
    for _ in slots:
        ys.append(y)
        y += d + COST_GAP
    bottom = ys[-1] + COST_R + COST_GAP
    ...
    parts.append('<rect x="%.1f" y="%.1f" width="%d" height="%.1f" fill="#00000033"/>'
                 % (R - COST_SHADE, top, COST_SHADE, bottom - top))
    ...
    for (kind, name), cy in zip(slots, ys):
        if kind == 'chain':
            ...
            continue
        glyphs.append(place_cost_glyph(name, cx, cy, icons))
    parts.extend(glyphs)
    return '\n'.join(parts)

def place_cost_glyph(name, cx, cy, icons):
    ret = []
    ret.append('<circle cx="%.1f" cy="%.1f" r="%d" fill="#000000" '
                   'opacity="0.42" filter="url(#softshadow)"/>'
                   % (cx + 3, cy + 4, COST_R))
    if isinstance(name, int):
        ret.append(_coin_badge(icons, cx, cy, COST_R, name))
    else:
        ret.append(icons.place(name, cx, cy, COST_R))
    return '\n'.join(ret)

def cost_banner(top, icons, badges, ribbon_color):
    '''Resource build-cost badges on a ribbon hanging down the left edge.'''
    return hang_ribbon(0, top, [('res', b) for b in badges], ribbon_color, icons)


def chain_in_column(top, icons, chain_in, ribbon_color, has_costs):
    ...
    return hang_ribbon(x0, top, [('chain', chain_in)], ribbon_color, icons)
...
def effect_medallion(icons, layout, cx, cy, r):
...
def load_wonders(path='cards.yaml'):
    data = yaml.safe_load(open(path, encoding='utf-8'))
    index = {}
    for w in data['wonders']:
        index[w['id']] = w
    return index
